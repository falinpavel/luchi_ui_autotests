...
class TestAllCategoriesIsPresent:
    ...
    @allure_mid_level_marks(
        story="Пользователь может переключаться между категориями в каталоге",
        testcase_id="Luchi-UI-001",
        title="Навигация по каталогу кликами по наименованиям категорий (табы)",
        label="WEB",
        owner="AQA Falin Pavel"
    )
    def test_swipe_all_category_tabs_on_catalog_page(self, category_name):
        luchi_app.home_page \
            .open_home_page() \
            .is_opened() \
            .accept_cookies()
